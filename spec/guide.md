 ...
 *   **위험:** 작업 실패 시 `git reset --hard`를 수행할 수 있습니다.
 ...
 *   **방어:** `mcp-client` 단계에서 접근 가능한 루트 디렉토리를 프로젝트 폴더 내부로 엄격하게 제한(Allowlist)하세요.
...
mkdir -p packages/core packages/orchestrator packages/memory packages/mcp-client packages/types
mkdir -p data/logs
```
...
### Step 3: 통합 테스트
 ...
 1.  루트에 테스트용 스크립트 작성.
 2.  의도적으로 에러를 발생시키는 코드를 `OperationExecutor`에 태워서, `git reset`이 제대로 작동하는지 검증합니다.
